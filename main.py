...
def get_menu_button_rects():
    ...
    total_w = btn_w * 3 + int(width * 0.04) * 2
    start_x = width // 2 - total_w // 2
    btn_y = int(height * 0.52)
    rects = []
    for i in range(3):
        x = start_x + i * (btn_w + int(width * 0.04))
        rects.append(pygame.Rect(x, btn_y, btn_w, btn_h))
    return rects


def draw_menu(hovered_index):
    screen.fill((30, 35, 50))
    tile = int(min(width, height) * 0.06)
    for r in range(height // tile + 1):
        for c in range(width // tile + 1):
            if (r + c) % 2 == 0:
                pygame.draw.rect(screen, (38, 44, 62),
                                 (c * tile, r * tile, tile, tile))
    panel_w = int(width * 0.55)
    panel_h = int(height * 0.22)
    panel_x = width // 2 - panel_w // 2
    panel_y = int(height * 0.14)
    pygame.draw.rect(screen, (50, 57, 80), (panel_x, panel_y, panel_w, panel_h), border_radius=24)
    pygame.draw.rect(screen, (180, 160, 100), (panel_x, panel_y, panel_w, panel_h), width=3, border_radius=24)
    title = hugeFont.render("Chess", True, (230, 210, 140))
    screen.blit(title, (width // 2 - title.get_width() // 2, panel_y + 20))
    subtitle = smallFont.render("Select Time Control", True, (200, 200, 200))
    screen.blit(subtitle, (width // 2 - subtitle.get_width() // 2, panel_y + panel_h - 55))
    try:
        wk = pygame.image.load("Asset/white-king.png").convert_alpha()
        bk = pygame.image.load("Asset/black-king.png").convert_alpha()
        icon_size = int(min(width, height) * 0.07)
        wk = pygame.transform.scale(wk, (icon_size, icon_size))
        bk = pygame.transform.scale(bk, (icon_size, icon_size))
        screen.blit(bk, (panel_x + 20, panel_y + 18))
        screen.blit(wk, (panel_x + panel_w - icon_size - 20, panel_y + 18))
    except Exception:
        pass
    rects = get_menu_button_rects()
    emoji_map = ["⚡", "🏃", "🎯"]
    label_map = ["Bullet", "Rapid", "Standard"]
    sub_map = ["1 minute", "5 minutes", "10 minutes"]

    for i, rect in enumerate(rects):
        is_hov = (i == hovered_index)
        shadow_rect = rect.move(4, 6)
        pygame.draw.rect(screen, (10, 10, 20), shadow_rect, border_radius=18)
        if is_hov:
            bg_color = (180, 155, 80)
            border_color = (240, 210, 120)
            text_color = (20, 20, 20)
        else:
            bg_color = (55, 63, 88)
        ...
        lbl = font.render(label_map[i], True, text_color)
        screen.blit(lbl, (rect.centerx - lbl.get_width() // 2,
                          rect.top + int(rect.height * 0.15)))
        sub = smallFont.render(sub_map[i], True,
                               (40, 40, 40) if is_hov else (160, 160, 160))
        screen.blit(sub, (rect.centerx - sub.get_width() // 2,
                          rect.top + int(rect.height * 0.58)))
    hint = smallFont.render("Click a mode to start", True, (120, 120, 140))
    screen.blit(hint, (width // 2 - hint.get_width() // 2, int(height * 0.72)))
def reset_game(time):
    global turn, selectedPiece, squareSel, previousPiece, inCheck, mated, dragging, original_col, original_row,promoting,promotingPawn,promotingColor,white_time,black_time,last_tick,time_out,in_menu
    turn = "white"
    CLOCK_TIME=time
    inCheck = False
    mated = False
...
while running:
    ...
    last_tick = now
    if in_menu:
        btn_rects = get_menu_button_rects()
        hovered = None
    ...
    if not mated:
        ...
        wt_rect.bottomright = (offset_x + dim * 10, offset_y + dim * 8)
        ...
        bt_rect.topright = (offset_x + dim * 10, offset_y)
